...
def pathfind(config, ob, actions, rr, cc):
    actions[nmmo.action.Move] = {
        nmmo.action.Direction: move.pathfind(config, ob, actions, rr, cc)
    }
...
def meander(realm, actions, entity):
    actions[nmmo.action.Move] = {
        nmmo.action.Direction: move.habitable(realm.map.tiles, entity)
    }
...
def evade(realm, actions, entity):
    actions[nmmo.action.Move] = {
        ...
        move.antipathfind(realm.map.tiles, entity, entity.attacker)
    }
...
def forageDP(realm, actions, entity):
    ...
    actions[nmmo.action.Move] = {
        nmmo.action.Direction: move.towards(direction)
    }
