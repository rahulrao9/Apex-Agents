...
class Item:
    def __init__(self, item_ary):
        index = scripting.Observation.attribute(item_ary,
                                                Serialized.Item.Index)
        self.cls = item.ItemID.get(int(index))

        self.level = scripting.Observation.attribute(item_ary,
                                                     Serialized.Item.Level)
        self.quantity = scripting.Observation.attribute(
            item_ary, Serialized.Item.Quantity)
        self.price = scripting.Observation.attribute(item_ary,
                                                     Serialized.Item.Price)
        self.instance = scripting.Observation.attribute(
            item_ary, Serialized.Item.ID)
        self.equipped = scripting.Observation.attribute(
            item_ary, Serialized.Item.Equipped)
...
class Scripted(nmmo.Agent):
    ...
    def explore(self):
        ...
        if self.timeAlive < sz // 4:
            ...
            move.explore(self.config, self.ob, self.actions, centR, centC)
    ...
    def __call__(self, obs):
        ...
        if self.config.PLAYER_DEATH_FOG is not None:
            step_per_tile = max(1, int(1 / self.config.PLAYER_DEATH_FOG_SPEED))
            start_running = self.timeAlive > self.config.PLAYER_DEATH_FOG - step_per_tile
...
class Gather(Scripted):
    ...
    def __init__(self, config, idx):
        super().__init__(config, idx)
        self.resource = [
            material.Fish, material.Herb, material.Ore, material.Tree,
            material.Crystal
        ]
