# core model for the PVP AI. It defines the neural network architecture used for decision making 
# in the NMMO environment.
# The model consists of several components, including encoders for the agent's own state, 
# allies, NPCs, and enemies, as well as an interaction block that allows for attention-based 
# interactions between these entities. The model also includes a memory block for maintaining 
# state across time steps and policy/value heads for action selection and value estimation.
import numpy as np
import torch
